...
public class PasswordController {
	...
	public String processForgotPasswordForm(
		...
		Optional<UsuarioModel> optional = userService
				.findUserByEmail(userEmail);

		if (!optional.isPresent()) {
			model.addAttribute("emailnaoexiste", true);
		} else {
			...
			passwordResetEmail
					.setText("Clique aki para gerar sua nova senha caralhuda\n"
							+ appUrl + ":9006/reset?token="
							+ user.getResetToken());
	...
	public String setNewPassword(Model model,
		...
		String aux = "";
			...
			Optional<UsuarioModel> user = userService
					.findUserByResetToken(requestParams.get("token"));
			...
			if (user.isPresent()) {
				...
				// Set new password
				resetUser.setSenha(bCryptPasswordEncoder.encode(requestParams
						.get("password")));
